...
class Conversation:
    ...
    def get_prompt(self, include_system = True) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_TWO:
            ...
            for i, (role, message) in enumerate(self.messages):
                try:
                    ...
                    continue
            return ret
        elif self.sep_style == SeparatorStyle.AGENT:
            ...
            for i, (role, message) in enumerate(self.messages):
                try:
                    ...
                    continue
            return ret.strip()
        elif self.sep_style == SeparatorStyle.AGENTV3:
            if include_system:
                ret = self.system + self.sep
            else:
                ret = ""
            for i, (role, message) in enumerate(self.messages):
                try:
                    if role in ['assistant', 'caller', 'conclusion']:
                        sep = self.sep2
                    else:
                        sep = self.sep
                    if message:
...
register_conv_template(
    Conversation(
        name="collab_agent_v1",
        system="A chat between a curious user and an artificial intelligence assistant who can use external tools and APIs to solve the user's question. "
        "The assistant gives tools and APIs calling processes or final answer to the human's question.",
        roles=("user", "assistant","caller", "observation"),
        messages=(),
        offset=0,
        sep_style=SeparatorStyle.AGENT,
        sep=" ",
        sep2="</s>",
    )
)
...
register_conv_template(
    Conversation(
        name="collab_agent_v3",
        system="A chat between a curious user and an artificial intelligence assistant who can use external tools and APIs to solve the user's question. "
        "The assistant gives tools and APIs calling processes or final answer to the human's question.",
        roles=("user", "assistant","caller", "observation", 'conclusion'),
        messages=(),
        offset=0,
        sep_style=SeparatorStyle.AGENTV3,
        sep=" ",
        sep2="</s>",
    )
)
